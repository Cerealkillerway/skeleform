...
Skeleform.utils.skeleformHandleResult = function(error, result, type, data, schema, item) {
    if (error) {
        if (error.error === 'unauthorized') {
            ...
            switch (error.reason) {
                case 'Email already exists.':
                    Materialize.toast(i18n.get('emailAlreadyUsed_error'), 5000, 'error');
                    break;
        ...
        switch (type) {
            case 'update':
            title = i18n.get('updateConfirm_msg');
            content = i18n.get('genericUpdate_msg');
            break;

            case 'create':
            title = i18n.get('insertConfirm_msg');
            content = i18n.get('genericInsert_msg');
...
Skeleform.utils.setFieldValue = function(instance, item, fieldSchema, isRestoring) {
