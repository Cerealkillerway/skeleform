...
Skeleform.utils.skeleformHandleResult = function(error, result, type, schema, formContext) {
    if (error) {
        if (error.error === 'unauthorized') {
            M.toast({
                html: Skeletor.Skelelang.i18n.get('permissions_error'),
                displayLength: 5000,
                classes: 'permissionsError'
            })
            ...
            switch (error.reason) {
                case 'Email already exists.':
                    //M.toast({html: Skeletor.Skelelang.i18n.get('emailAlreadyUsed_error'), displayLength: 5000, classes: 'error'})
                ...
                default:
                    M.toast({
                        html: Skeletor.Skelelang.i18n.get('serverError_error'),
                        displayLength: 5000,
                        classes: 'error'
                    })
            }
        ...
        // if custom success handler is defined, run it
        if (schema.handlers && schema.handlers.success !== undefined) {
            schema.handlers.success(type)
        }
        else  {
            // otherwise toast a success message
            // if a custom message is defined use it
            if (schema.customMessages && schema.customMessages[type] !== undefined) {
                content = schema.customMessages[type]
            }
            else {
                // otherwise use the default message
                switch (type) {
                    case 'update':
                    content = 'genericUpdate_msg'
                    break

                    case 'create':
                    content = 'genericInsert_msg'
                    break
                }
            }
            content = Skeletor.Skelelang.i18n.get(content)

            // clean form on creation
            if (type === 'create') {
...
Skeleform.utils.skeleformGatherData = function(formContext, fields) {
    ...
    fields.forEach(function(field) {
        ...
        if ($field.hasClass('skeleGather')) {
            ...
            Skeletor.SkeleUtils.GlobalUtilities.logger('Skipped field: ' + fieldSchema.name, 'skeleWarning')
        }
    })

    Skeletor.SkeleUtils.GlobalUtilities.logger('<separator>form gathered data:', 'skeleform')
...
Skeleform.utils.setFieldValue = function(instance, item, fieldSchema, formContext) {
    instance.view.autorun(function() {
        ...
        if (isActivated === true) {
            ...
            //Skeletor.SkeleUtils.GlobalUtilities.logger(fieldSchema.name + '-value: ' + value, 'skeleformCommon')
            //Skeletor.SkeleUtils.GlobalUtilities.logger(fieldSchema.name + '-getValue(): ' + instance.getValue(), 'skeleformCommon')
            //Skeletor.SkeleUtils.GlobalUtilities.logger('------------------', 'skeleformCommon')
            ...
            const validationOptions = fieldSchema.validation
...
Skeleform.utils.SkeleformStandardFieldValue = function(item, fieldSchema, instance) {
    ...
    if (data.replicaIndex !== undefined) {
        ...
        fieldData = Skeletor.SkeleUtils.GlobalUtilities.getPropertyFromString(name, replicaItem[index])
    }
    else {
        if (fieldSchema.i18n === undefined || fieldSchema.i18n === true) {
            fieldData = Skeletor.SkeleUtils.GlobalUtilities.getPropertyFromString(FlowRouter.getParam('itemLang') + '---' + name, item)
        }
        else {
            fieldData = Skeletor.SkeleUtils.GlobalUtilities.getPropertyFromString(name, item)
        }
    }
...
Skeleform.utils.createPath = function(path, data, schema, item) {
    ...
    function handleParam(param, type, value) {
        switch (param) {
            ...
            default:
            if (value === 'this') {
                let fieldSchema = Skeletor.SkeleUtils.GlobalUtilities.fieldSchemaLookup(schema.fields, param)
...
Skeleform.utils.autoSaveFormData = function(formContext, fields) {
    ...
    Skeletor.SkeleUtils.GlobalUtilities.logger('form autosave', 'skeleSuccess')
}
...
Skeleform.utils.restoreSavedData = function(formContext, savedDataIndex) {
    ...
    Skeletor.SkeleUtils.GlobalUtilities.logger('form restore from autosave', 'skeleform')
}
